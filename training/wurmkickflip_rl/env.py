...
class WurmkickflipEnv(gym.Env[np.ndarray, np.ndarray]):
    ...
    def step(self, action: np.ndarray):
        ...
        terminated = bool(contact_ratio < 0.35 or abs(float(self.board[2])) > 1.25)
        truncated = bool(self.time >= 20.0 or self.board[0] - prior_x > 100.0)
        if terminated:
            reward -= 2.0
        ...
        return self._observe(), reward, terminated, truncated, {"contact_ratio": contact_ratio}
