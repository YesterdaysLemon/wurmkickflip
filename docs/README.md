...
3. [`seed-forge.md`](seed-forge.md) — deterministic domain sampling, runtime effects, parity, and replay provenance.
4. [`forge-trials.md`](forge-trials.md) — held-out Wurm Olympics, causal controls, and replay handoff.
5. [`evolution-lab.md`](evolution-lab.md) — current evolution strategy and future contact-rich research.
6. [`policy-contract.md`](policy-contract.md) — canonical mounted-stunt and detached-locomotion contracts.
7. [`replay-artifacts.md`](replay-artifacts.md) — checksummed replay schema and playback rules.
8. [`asset-roadmap.md`](asset-roadmap.md) — primitive assets and future art needs.
9. [`agent-handoff.md`](agent-handoff.md) — change rules and verification checklist.
10. [`backlog.md`](backlog.md) — remaining work and completed milestones.
...
The browser runtime uses tracked JSON policies only. Browser ONNX Runtime and its WASM payloads have been retired; older PPO/ONNX and sinusoidal CPG tooling remains offline legacy research. Seed Forge starts nominally and can deterministically sample 14 environment, actuator, sensor, and board channels with TypeScript/Python parity; exact samples flow into replay provenance. Wurm Olympics runs the same canonical 60 Hz episode engine headlessly across 8–32 held-out seeds, compares neural/zero/frozen lanes, and hands every neural heat to timeline replay inspection. Deterministic replay capture/playback, granular config loading, reduced-motion behavior, collision properties, performance budgets, autonomous boarding interventions, and browser flows are covered by automated checks. Scene-graph telemetry records current and high-water worm-part counts so a stale/new rig overlap cannot hide during reset or stunt transitions; worm body cast shadows are disabled because the offset articulated shadow could look like a duplicate worm while riding.
