...
The Replay lab in `App.tsx` can start/finish a live capture, export stable validated JSON, import and verify JSON, play/pause/restart it, scrub the timeline, jump to discovery/contact/ride-contact events, change playback speed, and return to live simulation. Wurm Olympics sends any retained neural heat through the same validated player. A rejected or tampered import never replaces the active live/replay state. Scene playback uses recorded board/root poses, contacts, reward, and all 32 channels. Schema v1 does not store every segment pose, lifecycle/homeostasis state, or bowl inventory, so the visual adapter deterministically reconstructs the body from the recorded root and muscle channels and labels that limitation in the UI.
