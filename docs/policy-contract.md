...
Before v3 neural inference, both trainer and browser apply the same finite controller-input ranges: target bearings `[-1, 1]`, normalized distance `[0, 1.5]`, forward speed `[-2, 2]`, angular speed `[-3, 3]`, terrain friction `[0, 1.2]`, urgency/contact `[0, 1]`, slip `[0, 2]`, and obstacle components `[-1, 1]`. This sanitizes only the neural view; raw plant telemetry remains available to physics and evaluation. The artifact records these bounds in `training.controllerSensorClamps`.

V3 evolution canonicalizes fitness to four decimals before stable ranking and first-tie selection. Guard-band-safe, gate-safe, and infeasible candidates form descending lexicographic tiers; within every tier, guard-band count and then nominal gate count precede canonical fitness. Gate diagnostics retain eight decimals and ablation gates use raw rollout values. The serialized genome remains eight-decimal. Host-sensitive metrics, recovery values, ablation magnitudes, and their diagnostic trace hash are summary-only and never participate in policy artifact identity. The artifact records thresholds and gate booleans instead. Its canonical genome-recipe hash covers the model version, genome precision, trainer and articulated-plant sources, Python and JSON contracts, `pyproject.toml`, and `uv.lock`. Cross-host equality is checked outside the trainer rather than self-attested in metadata.

The `articulated-contact-v2` plant is part of the causal contract: target data can affect neural outputs but cannot directly alter root pose. Mean-free internal shape forces and equal/opposite distance constraints move a free 16-segment chain; net translation emerges only through anisotropic per-segment ground contact or obstacle impulses. Every tick performs one initial constraint solve plus exactly `postIntegrationRelaxationPasses` additional solves (three in this contract); collision projection alternates with the additional solves when geometry is present. The canonical JSON enumerates joint gains, spacing, actuator gains, constraint compliance, longitudinal/lateral friction, velocity limits, radii, ground clearance, and this relaxation count. The parser rejects drift from that contract; its `maximumVerticalSpeed` default exists only to read earlier schema-v2 artifacts that predate that field.

The tight Python/TypeScript transition fixture covers the obstacle-free articulated base plant, including joint mapping, Float32 commands, constraint count, and velocity limiting. Python training projects simplified circular obstacles in a vectorized surrogate. The browser uses swept, height-aware collision geometry and pins contacted particles during subsequent constraint relaxation. Those contact paths are intentionally not described as numerically identical: browser collision properties and contact-rich controller behavior have their own deterministic promotion gates.

`npm run verify:locomotion` requires deterministic active-contact travel, head-leading steering, local-sensor effects, stronger progress than zero action, frozen action, and a fixed segment shuffle, plus obstacle-free zero-friction center-of-mass conservation below `1e-12`. Each intervention replaces both the Float32 action delivered to the plant and the Float64 previous-command feedback committed for the next recurrent step. It also checks malformed schema-v1/v2 rejection and byte-identical same-seed smoke exports when identical warm-start bytes are staged at different paths. `npm run verify:gait` is the authoritative promotion exam for causal propagation and 900-tick recovery from a combined shove, segment lesion, and low-traction window. The long `npm run verify:locomotion:published` lane stages the retained causal candidate and repeats its exact four-member v3 confirmation generation in a unique temporary directory. Warm-start metadata contains only the source SHA-256 and model version; filesystem paths are not artifact identity.
